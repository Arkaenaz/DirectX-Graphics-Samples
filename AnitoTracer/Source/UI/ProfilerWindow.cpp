...
static void DrawProfilerTimeline(const ImVec2& size = ImVec2(0, 0))
{
	...
	ImVec2 scaledScreenPos = ImVec2(ImGui::GetCursorScreenPos().x + sizeActual.x, ImGui::GetCursorScreenPos().y + sizeActual.y);
	ImRect timelineRect(ImGui::GetCursorScreenPos(), scaledScreenPos);
	...
	ImVec2 cursor = ImVec2(timelineRect.Min.x + context.TimelineOffset.x, timelineRect.Min.y + context.TimelineOffset.y);
	ImVec2 cursorStart = cursor;
	ImDrawList* pDraw = ImGui::GetWindowDrawList();

	ImGuiID timelineID = ImGui::GetID("Timeline");
	timelineRect.Max = ImVec2(timelineRect.Max.x - style.ScrollBarSize, timelineRect.Max.y - style.ScrollBarSize);
	if (ImGui::ItemAdd(timelineRect, timelineID))
	{
		...
		*/
		...
		if (held)
			context.TimelineOffset = context.TimelineOffset + ImGui::GetIO().MouseDelta;
